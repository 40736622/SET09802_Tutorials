...
public class BlackJackv8 {
	public static void main(String[] args) {
		...
		final int COMPUTER_WINS = 1;
		final int PLAYER_WINS = -1;
		final int PLAY_ON = 0;
	...
	public static int offerUserAnotherGo(String[] playersCards, String[] computersCards, int maxCards) {
		String output = "Computer ";
		int value;
		for (int cardsInHand = 0; cardsInHand < maxCards; cardsInHand++) {
			if (computersCards[cardsInHand] != null) {
				// there is a card so display it
				if (cardsInHand == 0) {
					// do not display first card
					output = output + "??  ";
				} else {
					// this is not the first card
					output = output + computersCards[cardsInHand] + "  ";
				}
			}
		}
